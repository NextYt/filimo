...
import { ContentContext, ContentContextType, useContent, FilterOptions } from './ContentContext';
...
export function useMovieFilters() {
  ...
  const isIranianContent = useMemo(() => {
    return searchParams.get('country') === 'Iran' || 
  ...
  const setContentType = useCallback((type: "All" | "Movie" | "Series") => {
    ...
    if (type === 'All') {
      ...
      // Force a re-render if needed by dispatching another action with a small delay
      setTimeout(() => {
  ...
  const setHD = useCallback((value: boolean) => {
    dispatch({
      type: "SET_FILTERS",
      payload: { hd: value }
    });
  ...
  // Reset all filters - improved to ensure instant reset in one click
  ...
  const filterMovies = useCallback((movieData: CategorizedMovies): CategorizedMovies => {
  ...
  const memoizedSelectedSort = useMemo(() => state.selectedSort, [state.selectedSort]);
  const memoizedShowFilters = useMemo(() => state.showFilters, [state.showFilters]);
  ...
  return {
    ...
    toggleFilters: useCallback(() => dispatch({ type: "TOGGLE_FILTERS" }), [dispatch]),
    setSort: useCallback((sort: string) => dispatch({ type: "SET_SORT", payload: sort }), [dispatch]),
  };
}